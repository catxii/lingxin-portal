var bindApp = angular.module('rebindApp',[]);

bindApp.controller('ReBindCtl', ['$scope','$rootScope','$http','$timeout',function($scope,$rootScope,$http,$timeout) {
	$scope.sex = '0';
	$scope.idno = '';
	$scope.realName = '';
	$scope.schoolid="0";
   ...
   //查询年级信息
	...
	{
		$rootScope.user = {};
		$rootScope.user.type = userType;
		$rootScope.user.account = '1';
		$rootScope.user.password = '1';
		$rootScope.user.nickname='1';
	...
	teacherBind = function(){
		...
		.success(function(data){
			...
			{
				//完成注册
				var url = getPathUrl() + '/refreshInfo'
				$http({
					url:url,
					method:'get'
				})
				.success(function(){
					window.location = getPathUrl()+'/sys/home/index';
				});
			}
	...
	familyBind = function(){
		...
		.success(function(data){
			...
			else
			{
				...
				.success(function(){
					window.location =  getPathUrl() + '/mychildrens';
				});
			}
		});
	};
